...
[JT-073 2026-07-05 v1] Manual sync routes now orchestrate metadata sync through `EmailSyncService`, persist metadata-only raw-email rows without downgrading retained bodies, store broad candidate retained bodies when supported, store running and last-run status, and return typed not-configured responses when no Gmail connection is persisted.
[JT-074 2026-07-05] Manual sync now resolves the latest persisted Gmail connection metadata from local SQLite, while token refresh, product pages, and remaining backend pieces fill in over subsequent Phase 1 tickets.
...
The manual sync endpoint resolves the latest persisted Gmail connection metadata from local SQLite, returns a typed not-configured `400` until a Gmail connection is configured, rejects concurrent syncs with typed `409`, maps provider failures to email-specific `401`, `403`, `409`, `429`, `502`, or `503` responses with a `user_action` detail, and returns `EmailSyncStatus` after a run.
...
- Sync API test: `uv run pytest tests/test_sync_api.py -v` from `backend/` verifies manual sync and status routes, typed not-configured, concurrency, and email-provider errors, dependency-wired repositories, metadata persistence, and status-store behavior.
