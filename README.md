...
- Current Gmail auth-start endpoint: `GET /auth/gmail` returns a Google authorization URL, generated OAuth state, provider name, and the single `gmail.readonly` scope; `GET /auth/gmail/callback` exchanges the code, persists token material through `SecretStore`, and stores non-secret connection metadata. Provider-level token refresh reuses the stored refresh token and keeps raw tokens out of connection metadata.
- Current retained body fetching: Gmail can fetch normalized retained bodies for selected refs behind the provider seam, and manual sync stores retained bodies for broad job-search candidate messages after metadata persistence. Endpoint-driven access to arbitrary retained-body refs remains later Phase 1 work.
