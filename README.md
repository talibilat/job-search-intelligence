...
[JT-020 2026-07-05 v2] Gmail OAuth callback handling, token exchange, token persistence, retained-body fetching, incremental sync, product pages, and remaining backend pieces fill in over subsequent Phase 1 tickets.
Phase 0 (Groundwork) with early Phase 1 ingestion service slices.
[JT-066 2026-07-05 v2] The repository currently contains planning documents, root project metadata, the monorepo directory skeleton, the backend `uv` project scaffold, an initial FastAPI app factory (`backend/app/main.py`) with health, setup, provider config, local wipe-data, and Gmail auth-start routes, typed settings and API errors, the keyring-backed `SecretStore` adapter, provider registry, backend `LLMProvider` and `EmailProvider` Strategy interfaces, the backend `EmailSyncService` coordinator for paginated metadata sync and expired-cursor reconciliation, raw-email counting and idempotent upsert helpers on `EmailRepository`, deterministic backfill reconciliation metrics and service-level sync-state status snapshots in `backend/app/services/sync_service.py`, a provider-neutral broad job-search candidate query DTO and static signal factory, a Gmail provider helper for building read-only OAuth authorization URLs, the exported `GmailEmailProvider` skeleton with readonly scope validation and public-safe not-implemented runtime errors, retained email body HTML-to-text normalization, an async SQLite engine module with sqlite-vec loading, `vec_version()` verification, and Phase 0 connection PRAGMAs, shared SQLite URL parsing, an Alembic migration environment, shared SQLite repository helpers, Phase 0 repository stubs with table-shaped Pydantic record DTOs, the synthetic fixture DTO contract, sample fixture, and SQLite fixture loader, the backend OpenAPI schema generator, root pre-commit configuration, backend and frontend CI workflows, and the frontend Vite React TypeScript shell with Orval API client generation, Recharts foundation, route-query helpers, shared accessible UI primitives, setup and sync-readiness shell copy, empty `/dashboard`, `/insights`, and `/chat` route shells, npm API contract, typecheck, lint, Vitest, build gate scripts, and a Chromium Playwright smoke harness for the current browser shell.
[JT-066 2026-07-05 v2] The repository currently contains planning documents, root project metadata, the monorepo directory skeleton, the backend `uv` project scaffold, an initial FastAPI app factory (`backend/app/main.py`) with health, setup, provider config, local wipe-data, and Gmail auth-start routes, typed settings and API errors, the keyring-backed `SecretStore` adapter, provider registry, backend `LLMProvider` and `EmailProvider` Strategy interfaces, the backend `EmailSyncService` coordinator for paginated metadata sync and expired-cursor reconciliation, a provider-neutral broad job-search candidate query DTO and static signal factory, a Gmail provider helper for building read-only OAuth authorization URLs, the exported `GmailEmailProvider` adapter with readonly scope validation, public-safe not-implemented runtime errors for deferred OAuth callback, refresh, and body-fetch behavior, and `SecretStore`-backed safe full-backfill metadata listing, retained email body HTML-to-text normalization, an async SQLite engine module with sqlite-vec loading, `vec_version()` verification, and Phase 0 connection PRAGMAs, shared SQLite URL parsing, an Alembic migration environment, shared SQLite repository helpers, Phase 0 repository stubs with table-shaped Pydantic record DTOs, the `email_sync_state` migration, `SyncStateRepository`, `SyncService`, the synthetic fixture DTO contract, sample fixture, and SQLite fixture loader, the backend OpenAPI schema generator, root pre-commit configuration, backend and frontend CI workflows, and the frontend Vite React TypeScript shell with Orval API client generation, Recharts foundation, route-query helpers, shared accessible UI primitives, setup and sync-readiness shell copy, empty `/dashboard`, `/insights`, and `/chat` route shells, npm API contract, typecheck, lint, Vitest, build gate scripts, and a Chromium Playwright smoke harness for the current browser shell.
[JT-066 2026-07-05 v2] Gmail OAuth callback handling, token exchange, token persistence, retained-body fetching, incremental sync, product pages, the first application schema revision, and remaining backend pieces fill in over subsequent Phase 1 tickets.
[JT-066 2026-07-05 v2] Phase 0 (Groundwork) with early Phase 1 ingestion service slices.
[JT-066 2026-07-05 v2] The repository currently contains planning documents, root project metadata, the monorepo directory skeleton, the backend `uv` project scaffold, an initial FastAPI app factory (`backend/app/main.py`) with health, setup, provider config, local wipe-data, and Gmail auth-start routes, typed settings and API errors, the keyring-backed `SecretStore` adapter, provider registry, backend `LLMProvider` and `EmailProvider` Strategy interfaces, the backend `EmailSyncService` coordinator for paginated metadata sync and expired-cursor reconciliation, raw-email counting helpers on `EmailRepository`, deterministic backfill reconciliation metrics in `backend/app/services/sync_service.py`, a provider-neutral broad job-search candidate query DTO and static signal factory, a Gmail provider helper for building read-only OAuth authorization URLs, the exported `GmailEmailProvider` adapter with readonly scope validation, public-safe not-implemented runtime errors for deferred OAuth callback, refresh, and body-fetch behavior, and `SecretStore`-backed safe full-backfill metadata listing, retained email body HTML-to-text normalization, an async SQLite engine module with sqlite-vec loading, `vec_version()` verification, and Phase 0 connection PRAGMAs, shared SQLite URL parsing, an Alembic migration environment, shared SQLite repository helpers, Phase 0 repository stubs with table-shaped Pydantic record DTOs, the synthetic fixture DTO contract, sample fixture, and SQLite fixture loader, the backend OpenAPI schema generator, root pre-commit configuration, backend and frontend CI workflows, and the frontend Vite React TypeScript shell with Orval API client generation, Recharts foundation, route-query helpers, shared accessible UI primitives, setup and sync-readiness shell copy, empty `/dashboard`, `/insights`, and `/chat` route shells, npm API contract, typecheck, lint, Vitest, build gate scripts, and a Chromium Playwright smoke harness for the current browser shell.
[JT-066 2026-07-05 v2] The repository currently contains planning documents, root project metadata, the monorepo directory skeleton, the backend `uv` project scaffold, an initial FastAPI app factory (`backend/app/main.py`) with health, setup, provider config, local wipe-data, and Gmail auth-start routes, typed settings and API errors, the keyring-backed `SecretStore` adapter, provider registry, backend `LLMProvider` and `EmailProvider` Strategy interfaces, the backend `EmailSyncService` coordinator for paginated metadata sync, durable full-backfill page state, and expired-cursor reconciliation, a provider-neutral broad job-search candidate query DTO and static signal factory, a Gmail provider helper for building read-only OAuth authorization URLs, the exported `GmailEmailProvider` skeleton with readonly scope validation, public-safe not-implemented runtime errors, and a Gmail metadata lister for safe full-backfill pages, retained email body HTML-to-text normalization, an async SQLite engine module with sqlite-vec loading, `vec_version()` verification, and Phase 0 connection PRAGMAs, shared SQLite URL parsing, an Alembic migration environment, shared SQLite repository helpers, Phase 0 repository stubs with table-shaped Pydantic record DTOs, the `email_sync_state` and `email_backfill_state` migrations, `SyncStateRepository`, `BackfillStateRepository`, `SyncService`, `BackfillStateService`, the synthetic fixture DTO contract, sample fixture, and SQLite fixture loader, the backend OpenAPI schema generator, root pre-commit configuration, backend and frontend CI workflows, and the frontend Vite React TypeScript shell with Orval API client generation, Recharts foundation, route-query helpers, shared accessible UI primitives, setup and sync-readiness shell copy, empty `/dashboard`, `/insights`, and `/chat` route shells, npm API contract, typecheck, lint, Vitest, build gate scripts, and a Chromium Playwright smoke harness for the current browser shell.
[JT-066 2026-07-05 v2] The repository currently contains planning documents, root project metadata, the monorepo directory skeleton, the backend `uv` project scaffold, an initial FastAPI app factory (`backend/app/main.py`) with health, setup, provider config, local wipe-data, and Gmail auth-start routes, typed settings and API errors, the keyring-backed `SecretStore` adapter, provider registry, backend `LLMProvider` and `EmailProvider` Strategy interfaces, the backend `EmailSyncService` coordinator for paginated metadata sync, durable full-backfill page state, and expired-cursor reconciliation, raw-email counting helpers on `EmailRepository`, deterministic backfill reconciliation metrics in `backend/app/services/sync_service.py`, a provider-neutral broad job-search candidate query DTO and static signal factory, a Gmail provider helper for building read-only OAuth authorization URLs, the exported `GmailEmailProvider` skeleton with readonly scope validation and public-safe not-implemented runtime errors, retained email body HTML-to-text normalization, an async SQLite engine module with sqlite-vec loading, `vec_version()` verification, and Phase 0 connection PRAGMAs, shared SQLite URL parsing, an Alembic migration environment, shared SQLite repository helpers, Phase 0 repository stubs with table-shaped Pydantic record DTOs, the `email_sync_state` and `email_backfill_state` migrations, `SyncStateRepository`, `BackfillStateRepository`, `SyncService`, `BackfillStateService`, the synthetic fixture DTO contract, sample fixture, and SQLite fixture loader, the backend OpenAPI schema generator, root pre-commit configuration, backend and frontend CI workflows, and the frontend Vite React TypeScript shell with Orval API client generation, Recharts foundation, route-query helpers, shared accessible UI primitives, setup and sync-readiness shell copy, empty `/dashboard`, `/insights`, and `/chat` route shells, npm API contract, typecheck, lint, Vitest, build gate scripts, and a Chromium Playwright smoke harness for the current browser shell.
...
- Full metadata backfill state is stored in local SQLite by provider and account, including status, next page token, processed page and message counters, final replacement cursor, timestamps, and public-safe failure text only.
...
[JT-066 2026-07-05 v2] The backend has an initial FastAPI app factory, typed API error DTOs in `backend/app/api/errors.py`, setup, auth-start, and provider config DTOs/routes/services, the `app.providers.provider_registry` metadata and validation seam, the `app.providers.llm.LLMProvider` strategy seam, typed settings in `backend/app/config.py`, the `SecretStore` protocol, keyring adapter, and redaction helpers in `backend/app/security/`, the `EmailProvider` contract, broad candidate query DTO, `GmailEmailProvider` skeleton, Gmail metadata lister, retained body HTML normalizer, and Gmail read-only OAuth authorization URL helper in `backend/app/providers/email/`, the `EmailSyncService` metadata-page coordinator in `backend/app/services/sync_service.py`, an async SQLite engine with sqlite-vec setup in `backend/app/db/engine.py`, shared SQLite URL parsing in `backend/app/db/sqlite_url.py`, an Alembic migration environment in `backend/app/db/migrations/`, shared SQLite repository helpers and repository stubs in `backend/app/db/repositories/`, `SyncStateRepository` for persisted provider cursors, `BackfillStateRepository` for persisted full-backfill progress, table-shaped record DTOs in `backend/app/models/records.py`, `SyncService` for sync cursor business flow, `BackfillStateService` for resumable full-backfill state, synthetic fixture DTOs in `backend/app/models/synthetic_fixture.py`, the `SyntheticFixtureRepository` loader in `backend/app/db/repositories/synthetic_fixture.py`, a sample fixture in `backend/tests/fixtures/synthetic/basic_job_search.json`, `backend/scripts/generate_openapi.py` for deterministic OpenAPI schema generation, a `backend/pyproject.toml` with strict mypy defaults plus `uv` project metadata, `backend/pytest.ini`, and `backend/.env.example` documenting expected v1 operational settings.
[JT-066 2026-07-05 v2] The backend has an initial FastAPI app factory, typed API error DTOs in `backend/app/api/errors.py`, setup, auth-start, and provider config DTOs/routes/services, the `app.providers.provider_registry` metadata and validation seam, the `app.providers.llm.LLMProvider` strategy seam, typed settings in `backend/app/config.py`, the `SecretStore` protocol, keyring adapter, and redaction helpers in `backend/app/security/`, the `EmailProvider` contract, `GmailEmailProvider` skeleton, retained body HTML normalizer, and Gmail read-only OAuth authorization URL helper in `backend/app/providers/email/`, the `EmailSyncService` metadata-page coordinator, durable full-backfill page state, and deterministic backfill reconciliation metrics in `backend/app/services/sync_service.py`, provider-neutral candidate query DTOs, an async SQLite engine in `backend/app/db/engine.py`, shared SQLite URL parsing in `backend/app/db/sqlite_url.py`, an Alembic migration environment in `backend/app/db/migrations/`, shared SQLite repository helpers and repository stubs in `backend/app/db/repositories/`, raw-email counting helpers on `EmailRepository`, `SyncStateRepository`, `BackfillStateRepository`, table-shaped record DTOs in `backend/app/models/records.py`, synthetic fixture DTOs in `backend/app/models/synthetic_fixture.py`, the `SyntheticFixtureRepository` loader in `backend/app/db/repositories/synthetic_fixture.py`, a sample fixture in `backend/tests/fixtures/synthetic/basic_job_search.json`, `backend/scripts/generate_openapi.py` for deterministic OpenAPI schema generation, a `backend/pyproject.toml` with strict mypy defaults plus `uv` project metadata, `backend/pytest.ini`, and `backend/.env.example` documenting expected v1 operational settings.
[JT-066 2026-07-05 v2] The backend has an initial FastAPI app factory, typed API error DTOs in `backend/app/api/errors.py`, setup and provider config DTOs/routes/services, the `app.providers.provider_registry` metadata and validation seam, the `app.providers.llm.LLMProvider` strategy seam, typed settings in `backend/app/config.py`, the `SecretStore` protocol, keyring adapter, and redaction helpers in `backend/app/security/`, the `EmailProvider` contract, `GmailEmailProvider` skeleton, and retained body HTML normalizer in `backend/app/providers/email/`, the `EmailSyncService` metadata-page coordinator and durable full-backfill page state in `backend/app/services/sync_service.py`, an async SQLite engine with sqlite-vec setup in `backend/app/db/engine.py`, shared SQLite URL parsing in `backend/app/db/sqlite_url.py`, an Alembic migration environment in `backend/app/db/migrations/`, shared SQLite repository helpers and repository stubs in `backend/app/db/repositories/`, `SyncStateRepository` for persisted provider cursors, `BackfillStateRepository` for persisted full-backfill progress, table-shaped record DTOs in `backend/app/models/records.py`, `SyncService` for sync cursor business flow, `BackfillStateService` for resumable full-backfill state, synthetic fixture DTOs in `backend/app/models/synthetic_fixture.py`, the `SyntheticFixtureRepository` loader in `backend/app/db/repositories/synthetic_fixture.py`, a sample fixture in `backend/tests/fixtures/synthetic/basic_job_search.json`, `backend/scripts/generate_openapi.py` for deterministic OpenAPI schema generation, a `backend/pyproject.toml` with strict mypy defaults plus `uv` project metadata, `backend/pytest.ini`, and `backend/.env.example` documenting expected v1 operational settings.
...
[JT-066 2026-07-05 v2] The first Alembic schema revision creates `email_sync_state`, and the second creates `email_backfill_state`; run `uv run alembic upgrade head` before using sync-state or backfill-state repository behavior against a fresh local database.
...
[JT-066 2026-07-05 v2] - Backfill-state persistence test: `uv run pytest tests/test_backfill_state.py -v` from `backend/` verifies Alembic creates `email_backfill_state`, full backfill resumes from saved page tokens, final pages promote replacement sync cursors, and provider failures store public-safe error text.
