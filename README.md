...
[JT-103 2026-07-05] Phase 2 aggregation now has a deterministic `normalize_role_title()` service helper for role-title grouping keys; it changes no schema, API surface, classification prompts, or company normalization behavior.
...
- Role normalization service: `normalize_role_title()` in `backend/app/services/normalization.py` converts extracted titles into deterministic Phase 2 aggregation keys by removing seniority, title levels, location/work-arrangement noise, and punctuation, expanding common abbreviations, and preserving meaningful descriptors; `uv run pytest tests/test_role_normalization.py -v` verifies common variants.
