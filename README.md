...
[JT-020 2026-07-05 v1] The Alembic chain now includes `email_sync_state` followed by the core `raw_emails`, `email_classifications`, `applications`, `application_events`, `insights`, and `email_chunks` schema, and the migration environment loads sqlite-vec before applying vector-table revisions.
[JT-056 2026-07-05] Gmail OAuth callback handling now exchanges authorization codes, validates `gmail.readonly`, stores token material through the configured `SecretStore`, persists non-secret `email_connections` metadata, and returns only connection metadata.
[JT-062 2026-07-05] Gmail retained-body fetching now exists behind the provider seam for caller-selected refs; token refresh, retained-body repository writes, dashboard, insights, chat, and remaining backend pieces fill in over subsequent Phase 1 tickets.
The repository currently contains planning documents, the Phase 0 scaffold, Gmail read-only auth-start, safe metadata-only Gmail listing, retained-body fetching for caller-selected message refs, broad candidate selection before body retention, sync-service retained-body selection and chunking, retained-body raw-email repository writes, sync-state persistence, deterministic reconciliation metrics, the core SQLite schema, synthetic fixture tooling, backend and frontend CI, and the current React shell.
[JT-020 2026-07-05 v2] The repository currently contains the core schema migration, sqlite-vec migration loading, `SyncStateRepository`, `SyncService`, `GmailEmailProvider` metadata and retained-body seams, the provider-neutral candidate query DTO, the backend OpenAPI generator, and the frontend Vite React TypeScript shell with Orval, Recharts, route-query helpers, shared UI primitives, setup and sync-readiness copy, empty product route shells, npm gates, and Chromium Playwright smoke coverage.
[JT-020 2026-07-05 v1] The Alembic chain now includes `email_sync_state` followed by the core `raw_emails`, `email_classifications`, `applications`, `application_events`, `insights`, and `email_chunks` schema, and the migration environment loads sqlite-vec before applying vector-table revisions.
[JT-056 2026-07-05] Gmail OAuth callback handling now exchanges authorization codes, validates `gmail.readonly`, stores token material through the configured `SecretStore`, persists non-secret `email_connections` metadata, and returns only connection metadata.
[JT-062 2026-07-05] Gmail retained-body fetching now exists behind the provider seam for caller-selected refs; manual sync stores broad candidate retained bodies after metadata persistence; token refresh, dashboard, insights, chat, and remaining backend pieces fill in over subsequent Phase 1 tickets.
...
[JT-074 2026-07-05] Manual sync now resolves the latest non-reauth Gmail connection metadata from local SQLite, while token refresh, dashboard, insights, chat, and remaining backend pieces fill in over subsequent Phase 1 tickets.
...
[JT-078 2026-07-05] The setup page now loads setup status, starts the backend-built read-only Gmail OAuth flow, shows the `Continue to Google` authorization link, and reports callback completion from persisted non-secret Gmail connection metadata.
...
- Repository stubs: import `EmailRepository`, `EmailConnectionRepository`, `ApplicationRepository`, `EventRepository`, `InsightRepository`, `CorrectionRepository`, and `ChatRepository` from `app.db.repositories`; `uv run pytest tests/test_repository_stubs.py -v` verifies package exports, raw-email retention-state invariants, retained-body repr redaction, email connection upserts, and row-to-record mapping for table-shaped DTOs.
...
- Current setup shell: `GET /setup/status` returns typed first-run setup readiness fields, reports `gmail_connected` from the latest non-reauth `email_connections` metadata when available, safely reports disconnected before that table exists, and never returns secrets; `POST /setup` accepts non-secret first-run choices, validates selected provider metadata, and returns `{"status":"accepted",...}` without running provider auth flows or persisting secrets.
- Current Gmail auth endpoints: `GET /auth/gmail` returns a Google authorization URL, generated OAuth state, provider name, and the single `gmail.readonly` scope; `GET /auth/gmail/callback` exchanges the code, persists token material through `SecretStore`, stores non-secret `email_connections` metadata, and returns `EmailConnection`; token refresh and dashboard, insights, and chat pages remain later work.
