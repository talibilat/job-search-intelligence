...
- Sync service test: `uv run pytest tests/test_sync_service.py -v` from `backend/` verifies metadata-page coordination, expired history cursor fallback to full reconciliation, continuation page-token forwarding, ambiguity rejection when paginated sync includes a cursor without an explicit mode, retained-body selection for broad candidates plus explicit debugging or reconciliation refs, and provider-limit chunking.
[JT-062 2026-07-05 v2] - Sync service test: `uv run pytest tests/test_sync_service.py -v` from `backend/` also verifies retained-body selection for broad candidates plus explicit debugging or reconciliation refs, and provider-limit chunking.
...
- Current Gmail auth-start endpoint: `GET /auth/gmail` returns a Google authorization URL, generated OAuth state, provider name, and the single `gmail.readonly` scope; callback handling, token exchange, token persistence, endpoint-driven message sync, and repository writes for retained bodies remain later Phase 1 work.
[JT-062 2026-07-05 v2] - Current retained body fetching: Gmail can fetch normalized retained bodies for selected refs behind the provider seam; callback handling, token exchange, token persistence, endpoint-driven message sync, and repository writes for retained bodies remain later Phase 1 work.
