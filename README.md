...
[JT-103 2026-07-05] The backend now includes `normalize_role_title()` in `backend/app/services/normalization.py` for deterministic Phase 2 role grouping keys without schema or API changes.
...
- Role normalization test: `uv run pytest tests/test_role_normalization.py -v` from `backend/` verifies deterministic grouping keys for casing, punctuation, seniority labels, title levels, abbreviations, location noise, work-arrangement notes, and missing titles.
