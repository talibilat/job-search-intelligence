...
Current backend endpoints include `GET /health`, `GET /setup/status`, `POST /setup`, `GET|PUT /config/providers`, `POST /config/providers/llm/health`, `GET /auth/gmail`, `GET /auth/gmail/callback`, `POST /sync`, `GET /sync/status`, and `POST /local-data/wipe`.
...
The manual sync endpoint resolves the latest non-reauth Gmail connection from SQLite, runs resumable full backfill until `email_backfill_state` is completed and an incremental cursor is promoted, rejects concurrent syncs with typed `409`, maps provider failures to email-specific `401`, `403`, `409`, `429`, `502`, or `503` responses with a `user_action` detail, and returns `EmailSyncStatus` when runtime state can run.
...
The configured sync runtime wires Gmail provider access to SQLite repositories: it runs full backfill until a completed backfill has promoted an incremental cursor, then runs incremental sync on later manual runs.
...
- Email candidate query test: `uv run pytest tests/test_email_candidate_query.py -v` from `backend/` verifies broad job-search candidate signals, label exclusions, subject and normalized-body keyword matching, no body or snippet fields in the query, and that candidate filters stay out of provider metadata listing requests.
...
- Current LLM provider health API: `POST /config/providers/llm/health` validates selected non-secret LLM provider settings, delegates configured chat and embedding model checks to the injected `LLMProvider`, validates that the response covers the configured models, and maps LLM-provider failures to sanitized public error codes.
