...
Phase 0 (Groundwork).
[JT-069 2026-07-05 v2] Phase 0 remains the baseline, with the first Phase 1 sync-state persistence seam now in place.
[JT-069 2026-07-05 v2] The repository also includes the `email_sync_state` Alembic migration, `SyncStateRepository`, `SyncService`, and sync-state tests for provider-owned incremental cursors.
...
[JT-069 2026-07-05 v2] - Incremental sync anchors are stored in local SQLite as opaque provider cursor values scoped by provider and account; they are not OAuth token material or email content.
...
[JT-069 2026-07-05 v2] The first sync-state schema revision now creates `email_sync_state`; run Alembic upgrades before using repository code that expects migrated tables.
...
[JT-069 2026-07-05 v2] The first Alembic schema revision creates `email_sync_state`; run `uv run alembic upgrade head` before using sync-state repository behavior against a fresh local database.
...
[JT-069 2026-07-05 v2] - Sync-state persistence test: `uv run pytest tests/test_sync_state.py -v` from `backend/` verifies Alembic creates `email_sync_state`, cursors upsert per provider account, and repository reads require migrated schema.
