"""Add recurring feedback insight type.

Revision ID: 20260707_0190
Revises: 20260707_0192
...
revision: str = "20260707_0190"
down_revision: str | None = "20260707_0192"
...
def upgrade() -> None:
    _drop_staleness_triggers()
    with op.batch_alter_table("insights") as batch_op:
        batch_op.drop_constraint("ck_insights_type", type_="check")
        batch_op.create_check_constraint(
            "ck_insights_type",
            _in_values("type", NEW_INSIGHT_TYPES),
        )
    _create_staleness_triggers()


def downgrade() -> None:
    _drop_staleness_triggers()
    op.execute("DELETE FROM insights WHERE type = 'recurring_feedback'")
    with op.batch_alter_table("insights") as batch_op:
        batch_op.drop_constraint("ck_insights_type", type_="check")
        batch_op.create_check_constraint(
            "ck_insights_type",
            _in_values("type", OLD_INSIGHT_TYPES),
        )
    _create_staleness_triggers()


def _drop_staleness_triggers() -> None:
    for trigger_name in TRIGGER_NAMES:
        op.execute(f"DROP TRIGGER IF EXISTS {trigger_name}")


def _create_staleness_triggers() -> None:
    op.execute(
        """
