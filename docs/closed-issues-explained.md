...
There are backend endpoints for health, setup status, setup submission, provider config, Gmail auth start and callback, classification estimates, classification reprocessing plans, application detail, manual sync, sync status, and wiping local data.
...
There is a concrete Azure OpenAI chat-completions adapter, while the Ollama adapter remains pending.
