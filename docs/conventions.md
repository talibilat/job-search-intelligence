...
- Sync services persist full-backfill status, page tokens, counters, replacement cursors, and public-safe failure text through `BackfillStateRepository`, keyed by provider and account, without storing token material or email content in backfill state.
...
- Completed full backfills must clear the next page token, require a replacement provider cursor with issued timestamp, and promote that cursor to `SyncStateRepository` in the same local SQLite transaction that records the final page.
...
- Full-backfill callers must persist raw emails before recording page progress; listing a page with `run_backfill_page` must not advance counters or resume tokens by itself.
[JT-066 2026-07-05 v2] Full-backfill callers must persist raw emails before recording page progress; listing a page with `run_backfill_page` must not advance counters or resume tokens by itself.
...
- Public sync status crosses the API boundary through the `SyncJobStatus` DTO and must expose only phase, progress, deterministic counts, sanitized errors, and timestamps, never provider payloads, OAuth tokens, raw cursors, or email content.
...
- Gmail OAuth setup and auth work must follow `docs/google-oauth-setup.md`: user-created Desktop client, `gmail.readonly` only, provider-owned authorization URLs, and token material routed through `SecretStore` when callback/token handling lands.
