...
`GET /classification/reprocessing-plan` exposes the deterministic Phase 2 selection policy before runner work executes it: retained candidates are bucketed as up to date, unclassified, stale model, stale prompt version, or blocked by a missing configured target model for the selected LLM provider and `classification_prompt_version`.
