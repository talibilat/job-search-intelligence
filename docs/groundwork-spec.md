...
`InsightGenerationService` builds deterministic facts and cited evidence through `InsightInputBuilder`, calls the configured chat-capable `LLMProvider` only on cache miss or forced regeneration, validates that returned text cites at least one prepared `citation_id` and no fabricated citation-like IDs, raises a public-safe LLM response error for ungrounded content, and leaves the cached `insights` row unchanged when validation fails.
