...
- **Sync:** `POST /sync` runs manual metadata sync for a configured `EmailConnection`, stores metadata-only `raw_emails` without overwriting retained bodies, stores retained bodies for broad job-search candidates when the provider supports body fetching, persists provider cursors and resumable page progress, returns a typed not-configured `400` until Gmail connection persistence exists, and returns typed `409` for concurrent manual runs; `GET /sync/status` reports current or last-run state, provider, account, mode, timestamps, page count, message count, raw-email count, expired-cursor recovery, and public error text.
