...
- **`provider_configuration`** - one singleton row containing the selected Gmail and LLM providers, classification mode, sync settings, Azure endpoint and deployments, and Ollama URL and models.
  Credential values never enter this table and remain behind `SecretStore` references.
...
Public `POST /sync` and `GET /sync/status` route behavior is wired through the backend sync API and consumed by the overview page's manual sync action.
...
- **Setup/auth:** `GET /setup/status` derives setup completion from ready-to-sync and ready-to-classify states without exposing secrets; `POST /setup` and `PUT /config/providers` share the durable provider configuration service, store operational fields in the singleton SQLite configuration row, and accept write-only `SecretStr` credentials for Azure OpenAI and the Google Desktop OAuth client JSON; `GET /config/providers` returns only non-secret settings and credential requirement metadata; `GET /config/providers/readiness` reports Gmail sync, classification generation, embedding generation, and chat generation as `ready`, `missing_config`, `missing_credential`, `unavailable`, `reauth_required`, or `not_implemented`; `GET /auth/gmail` starts Gmail OAuth from SecretStore-backed client JSON and returns only the provider-built authorization URL, generated state value, and read-only Gmail scope; `GET /auth/gmail/callback` stores OAuth token material through `SecretStore` and returns only non-secret connection metadata.
  UI-persisted operational fields override environment-derived defaults after the singleton row exists.
  Environment variables remain bootstrap defaults and retain precedence for process-level settings such as the database location, secret-store backend, encryption key path, and fixed Gmail scope.
...
- **Sync:** `POST /sync` resolves the latest non-reauth Gmail connection metadata, runs full backfill until durable backfill state is completed and the replacement cursor is promoted, stores metadata-only `raw_emails` without overwriting retained bodies, persists `email_filter_decisions` audit rows for broad job-search filter outcomes using the same batch decision path as retained-body selection, stores retained bodies for broad job-search candidates when the provider supports body fetching, persists provider cursors and resumable page progress, returns a typed not-configured `400` when no Gmail connection is configured, returns typed `409` for concurrent manual runs, and maps email-provider failures to typed `401`, `403`, `409`, `429`, `502`, or `503` responses with email-specific error codes and a `user_action` detail; `GET /sync/status` reports current or last-run state, provider, account, mode, timestamps, page count, message count, raw-email count, retained-body fetch failure count, expired-cursor recovery, and public error text.
  ...
  [2026-07-09] Per-message retained-body fetch or decode failures are counted in `retained_body_failure_count` instead of aborting the whole sync run, so a poisoned Gmail message cannot pin a resumable backfill to one page forever.
  [2026-07-09] `GET /sync/recent-emails` returns public-safe raw-email metadata previews and defaults to `order=sent_at` for newest mailbox messages, with `order=ingested_at` available as a historical-backfill diagnostic view.
- **Pipeline status:** `GET /pipeline/status` returns deterministic local pipeline readiness without email body text: Gmail connection state, sync/backfill state, stage counts, unclassified retained candidates, last public-safe error, and a next-action enum for the UI.
- **Classification:** `GET /classification/estimate` returns retained candidate counts, estimated prompt and completion tokens, total estimated tokens, model and prompt version, token-estimate method, and cost availability before a bulk classification pass without calling an LLM or returning email content; `GET /classification/reprocessing-plan` returns read-only retained-candidate buckets for up-to-date, unclassified, stale-model, stale-prompt-version, and missing-target-model rows so prompt/model reruns are deterministic before a runner executes them; `POST /classification/run` executes one structured extraction batch for retained candidates through the configured provider, stores accepted classifications and run accounting, runs deterministic aggregation for accepted extraction facts, and returns accepted, malformed, application-upsert, event-upsert, skipped-not-job-related, and manual-conflict counts.
