...
- **Metrics (deterministic):** `GET /metrics/summary`, `/metrics/rates`, `/metrics/funnel`, `/metrics/timeseries`, `/metrics/breakdown?dimension=role|source|salary|tech|sponsorship|seniority|work_mode`, `/metrics/diagnostics`
  [JT-123 2026-07-07] Metrics routes will share the `MetricsFilter` DTO for status, source, sponsorship, first-seen UTC-normalized date range, trimmed non-blank role text, non-inverted salary band, and work-mode filters; this DTO does not add metrics endpoint behavior by itself.
