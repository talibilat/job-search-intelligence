...
- **Applications:** `GET /applications` (filters: status, source, sponsorship, date range, role, salary band, work_mode), `GET /applications/{id}`, `GET /applications/{id}/events`, correction endpoints for merge, split, status edit, and event edit.
  [JT-116 2026-07-06] The backend now implements the read-only detail slice, `GET /applications/{id}`, returning one `ApplicationRecord` from local SQLite and a typed `404` error when no application row matches; list, event, and correction routes remain later slices.
