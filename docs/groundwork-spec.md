...
[JT-191 2026-07-07] For `skill_gaps` Q-42, `InsightInputBuilder` scopes evidence to rejected applications, derives deterministic `rejected_skill_counts` from the cited rejected application `tech_stack` values, and the generation prompt instructs the provider not to treat interview or offer wins as gaps unless rejected-role evidence also supports the claim.
