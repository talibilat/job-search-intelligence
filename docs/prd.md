...
- **FR-2.7 Golden-set eval** *(P0, mandatory gate)* - ~30 private-data-free labeled emails + an accuracy report run whenever the classify prompt/model changes.
  *Accept:* `backend/evals/run_eval.py` prints precision/recall; Phase 2 gate is ≥90% precision AND ≥85% recall on job-vs-not; regressions block merges unless explicitly accepted.
