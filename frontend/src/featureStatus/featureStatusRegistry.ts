...
export const featureStatusRegistry: readonly FeatureStatusRecord[] = [
  ...
  },
  {
    ...
    connectedModules: ["Metrics summary API"],
    completedDate: "2026-07-05",
    dependencies: [
      "GET /metrics/summary",
    ...
    description:
      "Deterministic dashboard route with a Q-07 interview invitations card from the metrics summary API plus placeholders for remaining dashboard metrics.",
    endpoints: ["GET /metrics/summary"],
    files: [
      "frontend/src/pages/DashboardPage.tsx",
    ...
    howToUse: {
      expectedBehaviour:
        "The dashboard renders the Q-07 interview invitations count from the metrics summary API and keeps unrelated metrics marked as pending.",
      expectedSuccessResult:
        "QA can confirm the interview invitations card loads from deterministic event history without fabricated application counts.",
    ...
    implementationStatus:
      "Implemented as a Phase 3 Q-07 slice with no fabricated metrics.",
    ...
    screens: ["Dashboard"],
    sharedUi: ["ChartPanel"],
    stateConnections: ["Metrics summary load state", "Future route query filters"],
    status: "completed",
    testing: {
      canTestNow: true,
  ...
  },
  {
    ...
    howToUse: {
      expectedBehaviour:
        "The page renders an intentionally empty chat surface with a disabled message composer and clear copy that the agent is not connected yet.",
      expectedSuccessResult:
        "QA can confirm the route exists, does not send chat requests, and states that future answers must be grounded in cited applications, emails, or deterministic metric outputs.",
      navigationPath: "Primary navigation -> Chat",
      prerequisites: ["Frontend dev server"],
      qaValidationPoints: [
        "The composer textarea is disabled.",
        "The page says Chat agent work arrives in Phase 5.",
        "No backend chat request, provider call, or retrieval action is triggered by loading the route.",
      ],
      steps: [
        "Open /chat.",
        "Confirm the Chat unavailable status card is visible.",
        "Confirm the Message composer is disabled and explains the missing backend requirements.",
      ],
    },
    id: "frontend-chat-shell",
    implementationStatus:
      "Implemented as a Phase 0 route shell with streaming chat, persisted history, retrieval, and agent behaviour deferred to Phase 5.",
    name: "Chat route shell",
    relationship: [
